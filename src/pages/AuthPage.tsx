...
export default function AuthPage({ mode }: { mode: 'login' | 'signup' | 'reset-password' }) {
  ...
  useEffect(() => {
    if (currentMode === 'forgot-password' || currentMode === 'reset-password' || currentMode === 'signup-confirm') return;
    ...
    const initGoogleGsi = () => {
      ...
      google.accounts.id.initialize({
        ...
        callback: async (response: any) => {
          ...
          setSubmitting(true);
          try {
            const res = await signInWithIdToken(response.credential);
            if (!mounted) return;
            if (res.error) {
              setError(res.error);
            } else {
            ...
            if (mounted) setError(err?.message || "Google authentication failed.");
          } finally {
    ...
    // Poll for the google global variable if not loaded yet
  ...
  }, [currentMode, signInWithIdToken, navigate]);
              ...
              {currentMode !== 'forgot-password' && currentMode !== 'reset-password' && import.meta.env.VITE_GOOGLE_CLIENT_ID && (
                <>
                  ...
                  <div className="w-full flex flex-col items-center gap-3">
                    <div id="googleBtn" className="w-full min-h-[44px] flex justify-center" />
                    <button
                      type="button"
                      ...
                      className="hidden w-full flex items-center justify-center gap-3 bg-white/5 hover:bg-white/10 border border-ink-800 text-slate-200 font-medium py-3 rounded-xl transition-all duration-200 hover:border-ink-700"
                    >
