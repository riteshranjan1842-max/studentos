      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          <h1 className="text-3xl font-bold text-slate-900 dark:text-white flex items-center gap-3">
            <TrendingUp className="w-8 h-8 text-sky-500" /> CGPA Tracker
          </h1>
        ...
        {sgpas.length > 0 && (
          <div className="flex items-center gap-3 glass px-5 py-3 rounded-2xl border border-sky-500/30 bg-sky-500/5">
            <div>
              <p className="text-xs text-slate-500 font-medium">Cumulative CGPA</p>
              <h2 className="text-2xl font-bold mt-0.5 text-sky-700 dark:text-sky-400">
                {overallCgpa.toFixed(2)}
              </h2>
            </div>
            <div className="w-10 h-10 rounded-xl bg-sky-500/10 text-sky-700 dark:text-sky-400 flex items-center justify-center">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        ...
        <div className="md:col-span-1 space-y-4">
          <form onSubmit={addSgpa} className="glass rounded-2xl p-5 border border-ink-700/50 space-y-4">
            <h3 className="text-base font-semibold text-slate-900 dark:text-white">Add Semester Grade</h3>
            
            <div className="space-y-1.5">
              <label className="text-xs font-semibold text-slate-500 dark:text-slate-400 uppercase tracking-wider">Semester</label>
              <input
                value={semesterInput}
                onChange={(e) => setSemesterInput(e.target.value)}
                placeholder="e.g. Semester 1"
                className="w-full bg-ink-800 border border-ink-700 rounded-xl px-4 py-2.5 text-sm text-slate-900 dark:text-white placeholder-slate-500 focus:outline-none focus:border-sky-500 transition-colors"
                required
              />
            </div>

            <div className="space-y-1.5">
              <label className="text-xs font-semibold text-slate-500 dark:text-slate-400 uppercase tracking-wider">SGPA</label>
              <input
                value={sgpaInput}
        ...
        <div className="md:col-span-2 space-y-4">
          {loading ? (
            ...
            <div className="glass rounded-2xl p-12 text-center border border-ink-700/50">
              ...
              <h3 className="text-lg font-semibold text-slate-900 dark:text-white">No grades logged yet</h3>
            ...
            <div className="glass rounded-2xl p-5 border border-ink-700/50 space-y-4">
              <h3 className="text-base font-semibold text-slate-900 dark:text-white">Semester Summary</h3>
